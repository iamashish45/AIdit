...
// *** GRID FIX: This is the correct v1 import ***
...
const ColorPickerPopover = ({ color, onChange }) => {
  ...
  return (
    <div>
      ...
      {displayPicker ? ( <Box sx={popover}> <Box sx={cover} onClick={handleClose} /> <SketchPicker color={color} onChange={handleChange} /> </Box> ) : null}
    </div>
  );
};
...
function BackgroundTool() {
  ...
  const handleSubmit = async () => {
    ...
    setError('');
    const options = {};
    if (mode === 'color') options.bgColor = bgColor;
  ...
  const renderControls = () => {
    switch (mode) {
      ...
      case 'gradient':
        return (
          <Box sx={{ mt: 2 }}>
            {/* *** GRID FIX: Re-add the 'item' prop *** */}
            <Grid container spacing={2}>
              <Grid item> {/* <-- 'item' prop IS CORRECT */}
      ...
      case 'image':
        return ( <Box sx={{ mt: 2 }}> <Button variant="outlined" component="label" startIcon={<UploadFile />} disabled={isLoading}> Upload Background <input type="file" hidden accept="image/png, image/jpeg" onChange={handleBgImageChange} /> </Button> {bgImageFile && ( <Typography variant="caption" display="block" sx={{ mt: 1 }}> {bgImageFile.name} </Typography> )} </Box> );
      case 'remove':
      default:
  ...
  return (
    <Container maxWidth="md">
      <Paper elevation={3} sx={{ p: 4, width: '100%' }}>
        <Typography variant="h5" gutterBottom> AI Background Remover & Replacer </Typography>
        {!originalFile && ( <Box sx={{ border: '2px dashed #999', borderRadius: '8px', p: 4, textAlign: 'center' }}> <Button variant="contained" component="label" size="large" startIcon={<UploadFile />} disabled={isLoading}> Upload Image <input type="file" hidden accept="image/png, image/jpeg" onChange={handleFileChange} /> </Button> </Box> )}
        {originalFile && (
          // *** GRID FIX: Re-add the 'item' prop ***
          <Grid container spacing={3}>
            <Grid item xs={12} md={5}> {/* <-- 'item' prop IS CORRECT */}
              ...
              <Button variant="outlined" size="small" onClick={handleClear} startIcon={<Clear />} sx={{ mt: 1 }}> Clear Image </Button>
            </Grid>
            <Grid item xs={12} md={7}> {/* <-- 'item' prop IS CORRECT */}
              <Typography variant="h6">Background Options</Typography>
              <ToggleButtonGroup value={mode} exclusive onChange={handleModeChange} size="small" color="primary" sx={{ mb: 2 }} >
                <ToggleButton value="remove"><AutoFixNormal sx={{mr: 1}} /> Remove</ToggleButton>
                <ToggleButton value="color"><ColorLens sx={{mr: 1}} /> Color</ToggleButton>
                <ToggleButton value="gradient"><Gradient sx={{mr: 1}} /> Gradient</ToggleButton>
                <ToggleButton value="image"><Photo sx={{mr: 1}} /> Image</ToggleButton>
              </ToggleButtonGroup>
              <Divider />
              {renderControls()}
              <Button variant="contained" color="primary" size="large" onClick={handleSubmit} disabled={isLoading} sx={{ mt: 3 }}>
                {isLoading ? 'Processing...' : 'Process Image'}
              </Button>
            </Grid>
          </Grid>
        )}
        {isLoading && ( <Box sx={{ width: '100%', my: 2 }}> <LinearProgress variant="determinate" value={uploadProgress} /> </Box> )}
        {error && <Alert severity="error" sx={{ my: 2 }}>{error}</Alert>}
      </Paper>
      <PreviewModal open={modalOpen} onClose={() => setModalOpen(false)} original={originalImageURL} processed={processedImageURL} />
    </Container>
  );
}
